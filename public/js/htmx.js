...
var htmx = (() => {
  ...
  class Htmx {
    ...
    #verbs = ['get', 'query', 'post', 'put', 'patch', 'delete'];
    #hxOnQuery;
    #transitionQueue;
    ...
    constructor() {
      ...
      this.#actionSelector = `[${this.#prefix('hx-action')}],[${this.#prefix('hx-get')}],[${
        this.#prefix('hx-post')
      }],[${this.#prefix('hx-put')}],[${this.#prefix('hx-patch')}],[${this.#prefix('hx-delete')}]`;
    ...
    #attributeValue(elt, name, defaultVal, eltCollector) {
      ...
      let inheritName = name +
        (this.config.implicitInheritance ? '' : this.#maybeAdjustMetaCharacter(':inherited'));
    ...
    #boostedMethodAndAction(elt, evt) {
      if (elt.matches('a')) {
        ...
        let method = evt.submitter?.getAttribute?.('formMethod') || elt.getAttribute('method') ||
          'GET';
        return { action, method };
      }
    }
          ...
          validate:
            'true' ===
              this.#attributeValue(
                sourceElement,
                'hx-validate',
                sourceElement.matches('form') && !sourceElement.noValidate &&
    ...
    async #handleTriggerEvent(ctx) {
      ...
      ctx.request.headers['HX-Request-Type'] = (ctx.target === document.body || ctx.select)
        ? 'full'
        : 'partial';
      if (ctx.target) {
    ...
    #maybeBoost(elt) {
      ...
      if (boostValue && boostValue !== 'false' && this.#shouldBoost(elt)) {
        elt._htmx = {
          eventHandler: this.#createHtmxEventHandler(elt),
          requests: [],
          boosted: boostValue,
        };
        elt.setAttribute('data-htmx-powered', 'true');
    ...
    #makeFragment(text) {
      let response = text.replace(/<hx-([a-z]+)(\s+|>)/gi, '<template hx type="$1"$2').replace(
        /<\/hx-[a-z]+>/gi,
        '</template>',
      );
      let title = '';
    ...
    #processPartials(fragment, ctx) {
      ...
      for (let templateElt of fragment.querySelectorAll('template[hx]')) {
        ...
        if (type === 'partial') {
          let target = templateElt.getAttribute(this.#prefix('hx-target')) ||
    ...
    #handleAnchorScroll(ctx) {
      if (ctx.request?.anchor) {
        document.getElementById(ctx.request.anchor)?.scrollIntoView({
          block: 'start',
          behavior: 'auto',
        });
      }
    ...
    #processMainSwap(ctx, fragment, partialTasks) {
      ...
      if (
        swapSpec.style === 'delete' || fragment.childElementCount > 0 ||
        /\S/.test(fragment.textContent) || !partialTasks.length
      ) {
        if (ctx.select) {
    ...
    async #insertContent(task, cssTransition = true) {
      ...
      if (swapStyle === 'innerHTML' || (swapStyle === 'outerHTML' && parentNode)) {
        ...
        settleTasks = cssTransition && settleDelay
    ...
    #findAllExt(eltOrSelector, maybeSelector, thisAttr, global) {
      ...
      let parts = selector
        ? selector.replace(/<[^>]+\/>/g, (m) => m.replace(/,/g, '%2C'))
            ...
            if (bestMatch !== insertionPoint) {
              ...
              while (cursor && cursor !== bestMatch) {
                ...
                if (
                  ...
                  (ctx.idMap.has(tempNode) || this.#matchesUpcomingSibling(ctx, tempNode, newChild))
                ) {
    ...
    #copyAttributes(destination, source) {
      ...
      for (const attr of source.attributes) {
        if (
          ...
          if (
            attr.name === 'value' && destination instanceof HTMLInputElement &&
